    ...
    >
      <a href={href}>
        <div className="flex items-center gap-x-4">
          ...
          >
            <span id="item-border" className="absolute top-0 left-0 w-full h-full">
...
const MobileNav: React.FC<MobileNavProps> = ({ withNavbar = true }) => {
  ...
  const getActiveAngularStep = useCallback(
    (radOffset: number) => {
      return (
        Math.round(mod(radOffset, 2 * Math.PI) / ANGLE_STEP_RAD) % DISPLAY_LINKS_LENGTH
      );
    },
    [ANGLE_STEP_RAD],
  );
            ...
            >
              {Array.from({ length: REPS })
                ...
                .map((link: { label: string; href: string }, idx, arr) => {
                  return (
                    <NavItem
                      active={(arr.length - activeIndex) % arr.length === idx && open}
                      key={link.label + idx}
                      label={link.label}
                      href={link.href}
                    />
